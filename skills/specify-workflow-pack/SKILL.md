...
description: Create, install, sync, and validate a reusable Specify-first workflow pack across repositories and workspaces. Use when Codex needs to standardize a repo around AGENTS.md, CLAUDE.md, constitution, Specify templates, multi-agent prompts, required skills, brief-first delivery, or machine-portable governance shared across Codex, Claude Code, OpenCode, GitHub Copilot, and Antigravity.
---
...
- keeping Codex, Claude Code, OpenCode, Copilot, and Antigravity aligned on the same repo-local process
...
- existing `skills/`, `.claude/skills/`, `.opencode/skills/`, or `.agents/skills/` fallbacks
...
The installer renders repo-local files from the templates in `assets/templates/`, creates agent-native entrypoints for Codex, Claude Code, OpenCode, and GitHub Copilot, and copies required fallback skills into `skills/` plus the compatible mirror directories for other agents.
...
- required skills are available globally or in the repo-local fallback mirrors
